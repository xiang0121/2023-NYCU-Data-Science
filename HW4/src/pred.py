...
class TestDataset(Dataset):
    def __init__(self, root) -> None:
        super().__init__()
        self.root = root
        self.img_list = sorted(glob.glob(os.path.join(self.root, '*.jpg')))
    ...
    def __getitem__(self, index):
        img = torchvision.io.read_image(self.img_list[index], torchvision.io.image.ImageReadMode.RGB) / 255
        img = torchvision.transforms.functional.normalize(img, [0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
        return img
...
def test(model, test_loader, device):
    ...
    for (image, _) in tqdm(test_loader):
        with torch.no_grad():
            image = image.to(device)
            output = model(image)
            res.append(torch.sum(output).item())
    return res

def main():
    print('WARNING: Please check the test data folder path should be \'../test\'')
    ...
    test_loader = torch.utils.data.DataLoader(testset, batch_size=1, shuffle=False, num_workers= os.cpu_count())
    model.eval()
    res = []
